...
void level_II_Update(levelII* l, ALLEGRO_KEYBOARD_STATE* keystate){
    ...
    if(!l->inPause){
        ...
        if(!l->player.modoAtaque && !l->player.modoDefesa){
         ...
         if (!check_entity_tile_collision(&l->player, l->map, l->tileset, 1)||!check_entity_tile_collision(&l->player, l->map, l->tileset, 7)|| !check_entity_tile_collision(&l->player, l->map, l->tileset, 3)) {    
        ...
        if(l->player.x < 353) l->player.x = 353;
        if(l->player.x > 2743-l->player.width) l->player.x = 2743-l->player.width;
...
void Level_II_Draw(levelII l, ALLEGRO_FONT* Font){
    ...
    draw_tilemap(l.map, l.tileset, l.cameraX - 50, l.cameraY);
    drawHud(&l.hud);
